...
public class TestAPI_Query extends APITestbase {
		...
		for (ExampleWiki lwiki : getWikis()) {
			if (debug) {
				...
				LOGGER.log(Level.INFO,
						String.format("wiki %s:%s%s", lwiki.getWikiId(), llwiki.siteurl, llwiki.scriptPath));
			}
			...
			if (titles.size() > 0) {
				...
				for (Page page : pages) {
					...
					if (debug) {
						LOGGER.log(Level.INFO, lwiki.wiki.getSiteurl() + ":" + page.getTitle());
					}
					assertEquals(lwiki.wiki.getSiteurl(), expected.getTitle(), page.getTitle());
					// FIXME add to interface
					assertTrue(lwiki.wiki.getSiteurl() + "/" + lwiki.getMediaWikiJapi().getScriptPath() + ":"
	...
	public void testImageInfo() throws Exception {
		...
		assertEquals("http://wiki.bitplan.com/images/wiki/a/ae/Index.png", ii.getUrl());
	...
	/*
	 * @Test public void testParamEncoding() throws Exception { String
	 * ctbe="!#$&'()*+,/:;=?@[]|"; Mediawiki lwiki = (Mediawiki)
	 * ewm.get("targetWiki").wiki; String pencoded=lwiki.paramEncode(ctbe); String
	 * encoded=lwiki.encode(ctbe); System.out.println(pencoded);
